...
# htseq-count
#' 
#' @param counttable A dataframe of RNA-seq read count data, such as that generated htseq-count. The row should be gene names.
#'
#' @param group1 The number of samples in the first comparison group (e.g. males). 
#'
#' @param group2 The number of samples in the second comparison group (e.g. females).
#' 
#' @param FDRthresh The Benjamini-Hochberg false discovery rate value to use as a significance threhold for calling genes differentially expressed (default is 0.01). Only used if fullResults=FALSE.
#' 
#' @param fullResults A parameter that specifies whether to return a dataset that includes the results for all genes regardless of significance (TRUE) or only the results that meet the significance threshold across all three analyses (FALSE). Default value is FALSE.
#'
#' @return The log2FC and B-H FDR values from DESeq2, limma, and edgeR, either for all genes (fullResults=TRUE) or only genes that meet the significance threshold (fullResults=FALSE).
#'
#' @examples 
#' allOutput <- runDGE(counttable, 5, 5, fullResults=TRUE)
